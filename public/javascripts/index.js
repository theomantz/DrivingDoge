...
function clearData() {
  ...
  d3.selectAll(".chart-metrics-table, .default-metrics, .chart")
    ...
    .style("font-size", "0px")
    .remove();
  d3.select("svg").transition().duration(750).style("opacity", 0).remove();
}
...
async function init(width, height) {
  ...
  d3.select("#svg-container").attr(
    "style",
    `min-width:${width * 0.6}px;min-height:${height + 200}px`
  );

  d3.select("#chart-metrics").style("min-height", `${13 * rems}px`);

  d3.select("#right-sidenav").style("min-width", `${width * 0.2}px`);
